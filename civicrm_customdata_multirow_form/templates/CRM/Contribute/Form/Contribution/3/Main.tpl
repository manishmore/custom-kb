...
{/if}
    ...
    <div class="crm-group custom_pre_profile-group">
    	{include file="CRM/UF/Form/3/Block.tpl" fields=$customPre} 	
    </div>

{foreach from=$cgrouptempArr item=cgi}	
    <fieldset>
        <legend id="myCustomGroup_{$cgi}">{$customGroupName[$cgi]}</legend>
        <a id="aFocus{$cgi}" href="#"></a>   
        <table id="myTable{$cgi}">
        <tbody>
        <thead>
        {foreach from=$labelArray[$cgi] item=cname}
          <td>{$cname}</td>
        {/foreach}
        </thead>
        {if $cuArr}
        {foreach from=$cuArr[$cgi] item=cu}
        <tr>
          {foreach from=$cu key=k item=c}
        	<td><input type="text" name="custom_{$k}[]" value="{$c}" size="28" class="form-text big"/></td>
         {/foreach} 
         </tr>
        {/foreach}
        {else}
        <tr>
        {foreach from=$tempArray[$cgi] item=cname}
          <td>{$form.$cname.html}</td>
        {/foreach}
        </tr>
        {/if}
        </tbody>
        ...
        <tbody>
        <tr>
            <td><a href="#" id="somebutton{$cgi}">Add new row</a></td>
        </tr>
        </tbody>
        </table>
    </fieldset>
    ...
    {/literal}
    {foreach from=$tempArray[$cgi] item=fieldname}
    {literal} var fieldNameElement = '#{/literal}{$fieldname}{literal}';
    var newFieldName = '{/literal}{$fieldname}{literal}'+'[]';
    cj(fieldNameElement).attr( "name" , newFieldName );{/literal}
    {/foreach}
    {literal}
    cj('#somebutton{/literal}{$cgi}{literal}').click(function(){
        cj('#myTable{/literal}{$cgi}{literal} tbody>tr:last').clone(true).insertAfter('#myTable{/literal}{$cgi}{literal} tbody>tr:last').find('input').val('');
        //cj('#myTable{/literal}{$cgi}{literal} tbody>tr:last').scrollIntoView(true);
        document.getElementById('aFocus{/literal}{$cgi}{literal}').focus();  
        //document.getElementById( "myTable{/literal}{$cgi}{literal}" ).html();
        return false;
      });
    });
    </script>
