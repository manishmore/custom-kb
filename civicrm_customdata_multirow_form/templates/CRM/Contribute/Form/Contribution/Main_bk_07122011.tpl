...
{/if}
    ...
    {/if} 
{if $Displayform ==1}
<fieldset><legend id="myCustomGroup">{$customGroupName}</legend> 
<table id="myTable">
<tbody>
<thead>
{foreach from=$labelArray item=cname}
  <td>{$cname}</td>
{/foreach}
</thead>
{if $cuArr}
{foreach from=$cuArr item=cu}
<tr>
...
{else}
<tr>
{foreach from=$tempArray item=cname}
...
<tbody>
<tr>
    <td><a href="#" id="somebutton">Add new row</a></td>
</tr>
</tbody>
</table>
</fieldset>
...
{/literal}
{foreach from=$tempArray item=fieldname}
...
cj('#somebutton').click(function(){
    cj('#myTable tbody>tr:last').clone(true).insertAfter('#myTable tbody>tr:last').find('input').val('');
    cj('#myTable tbody>tr:last').focus();
  });
});
</script>
{/literal}
